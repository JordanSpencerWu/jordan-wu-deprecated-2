...
Looking at the `A minor` scale we can see all the notes in the scale are the white keys just like `C major`. The reason why is because they are relative to each other where all the notes are the same in both scales. The only difference is the tonic!
...
When you pick a scale, most of the notes in the melody and chords should be within the scale. The four commmonly used modes are major, minor, dorian, and mixolydian.
