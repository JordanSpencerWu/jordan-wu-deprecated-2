...
Ok now we have everything we need to write our math equation! However, Gatsby react component loads faster then our MathJax library. To fix this we need to tell our React component to invoke MathJax once it's loaded using the `useEffect` hook. We would add this hook to the React that renders our markdown file.
